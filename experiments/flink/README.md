...
A collection of real-time applications built with Apache flink.
...
```bash
$ git clone git@github.com:mayconbordin/flink-applications.git
$ cd flink-applications/
$ mvn -P<profile> package
```

Use the `local` profile to run the applications in local mode or `cluster` to run in a remote cluster.

If you are going to use the [flink](bin/flink) (do not confuse with the [`flink`](http://flink.apache.org/documentation/Command-line-client.html) command line client) script to submit topologies to a cluster, you must also install the python requirements:
...
```bash
$ bin/flink-submit <jar> <application-name> (local|remote) [OPTIONS...]
```

Example:

```bash
$ bin/flink-submit target/flink-applications-*-with-dependencies.jar word-count local
```
...
Topologies are placed in the `flink.applications.topology` package and they must
...
`flinkTopology`. The `getLogger` method is used in case the `AbstractTopology`
...
Each application will have its own constants interface at the `flink.applications.interfaces`
...
Spouts are placed in the `flink.applications.spout` package and they must extend
...
Bolts are placed in the `flink.applications.bolt` package and they must extend
...
Sinks are placed in the `flink.applications.sink` package and they must extend
...
specific package inside the `flink.applications.model` package.

And if the source-code is just an utility, place it in the `flink.applications.util`
package.
