      ...
      for(ct1 in myseq){
        for(ct2 in myseq){
            ...
            if(file.exists(mypath)) {
              ...
              current_arg<-sprintf("application:%s, opt:%s, batch:%s, %s, %s",app,opt,bt,ct1,ct2)
    ...
    if(app=="5"){
      for(ct1 in myseq){
          ...
          if(file.exists(mypath)) {
            setwd(mypath)
            current_time<-scan("sink.txt",nlines=1, quiet=TRUE)
            current_arg<-sprintf("application:%s, opt:%s, batch:%s, %s",app,opt,bt,ct1)
            if(min_execution_time >current_time){
              min_execution_time<-current_time
              min_argument<-current_arg
            }
        ...
        for(ct2 in myseq){
          for(ct3 in myseq){
              ...
              if(file.exists(mypath)) {
                setwd(mypath)
                current_time<-scan("sink.txt",nlines=1, quiet=TRUE)
                current_arg<-sprintf("application:%s,  opt:%s, batch:%s, %s, %s, %s",app,opt,bt,ct1,ct2,ct3)
