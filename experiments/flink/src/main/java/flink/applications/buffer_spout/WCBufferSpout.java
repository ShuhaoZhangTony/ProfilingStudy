package flink.applications.buffer_spout;
...
public class WCBufferSpout extends AbstractSpout {
    private static final Logger LOG = LoggerFactory.getLogger(WCBufferSpout.class);
    ...
    public void nextTuple() {
        ...
        if (index_e < end_index) {
            for (int i = 0; i < batch; i++) {
                valueb.append(array[index_e % array.length]);
                index_e++;
            }
        }
        ...
        if (value != null) {
            ...
            tuples = parser.parse(value.toString());
            if (tuples != null) {
                for (StreamValues values : tuples) {
