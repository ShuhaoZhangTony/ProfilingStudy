 ...
 * @author mayconbordin
 */
public class NullSink_WC extends BaseSink {
    ...
    public void initialize() {
        LOG = LoggerFactory.getLogger(NullSink_WC.class);
        start_true = 0;
        end = 0;
        end_index = 0;
        //loadConfig();
        ...
        if (!input.getValues().isEmpty()) {
            ...
            if (index_e == end_index) {//320M
                ...
                try {
                    FileWriter fw;
                    try {
//                        int workthread = config.getInt(WordCountConstants.Conf.SPLITTER_THREADS, 1);
//                        if ((boolean) config.get("default_setting")) {
//                            fw = new FileWriter(new File(config.getString("metrics.output") + "\\sink.txt"));
//                        } else
//                            fw = new FileWriter(new File(config.getString("metrics.output") + "\\sink_"
//                                    + workthread + "_" + config.getInt("batch") + ".txt"));
//
...
//    			}
                    // LOG.info("Finished execution:" + (end - start_true) / 1000000000);
...
//				}
                ...
                //FlinkClient client = FlinkClient.getConfiguredClient(conf).getClient();
                FlinkClient client = new FlinkClient(conf, "localhost", 6123);
                try {

                    client.killTopology("word-count"); //provide topology name
                    System.exit(0);
