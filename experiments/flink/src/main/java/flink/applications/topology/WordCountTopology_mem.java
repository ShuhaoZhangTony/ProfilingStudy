...
import flink.applications.bolt.WordCountBolt_mem;
...
public class WordCountTopology_mem extends BasicTopology {
    private static final Logger LOG = LoggerFactory.getLogger(WordCountTopology_mem.class);

    private int splitSentenceThreads;
    private int wordCountThreads;

    public WordCountTopology_mem(String topologyName, Config config) {
    ...
    public FlinkTopology buildTopology() {
        ...
        if (config.getInt("batch") == -1) {
            builder.setBolt(Component.COUNTER, new WordCountBolt_mem(), wordCountThreads)
