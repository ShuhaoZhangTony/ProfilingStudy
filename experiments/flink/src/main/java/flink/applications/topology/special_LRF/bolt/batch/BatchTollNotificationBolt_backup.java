...
import flink.applications.topology.special_LRF.types.internal.LavTuple;
...
/**
 * {@link BatchTollNotificationBolt_backup} calculates the toll for each vehicle and reports it back to the vehicle if a vehicle
 ...
 * <br />
 * {@link BatchTollNotificationBolt_backup} processes four input streams. The first input is expected to be of type
 ...
 * @author mjsax
 */
public class BatchTollNotificationBolt_backup extends BaseRichBolt {
    private final static long serialVersionUID = 5537727428628598519L;
    private static final Logger LOGGER = LoggerFactory.getLogger(BatchTollNotificationBolt_backup.class);
    /**
    ...
    public void execute(Tuple tuple) {
        //get input:
        LinkedList<Object> input_l = (LinkedList<Object>) tuple.getValue(0);
        int batch_size = input_l.size();
        ...
        for (int i = 0; i < batch_size; i++) {
            if (inputStreamId.equals(TopologyControl.POSITION_REPORTS_STREAM_ID)) {
                ...
                this.inputPositionReport.addAll((PositionReport) input_l.get(i));
...
//                    return;
                ...
                //TOLL_NOTIFICATIONS_STREAM.add(tollNotification);

            } else if (inputStreamId.equals(TopologyControl.ACCIDENTS_STREAM_ID)) {
                this.inputAccidentTuple.clear();
                this.inputAccidentTuple.addAll((AccidentTuple) input_l.get(i));
...
//            assert (this.inputAccidentTuple.getMinuteNumber().shortValue() == this.currentMinute);
                ...
                this.inputCountTuple.addAll((CountTuple) input_l.get(i));
...
//            assert (this.inputCountTuple.getMinuteNumber().shortValue() - 1 == this.currentMinute);
                ...
                this.inputLavTuple.addAll((LavTuple) input_l.get(i));
...
//        }
//        if (TOLL_NOTIFICATIONS_STREAM.size() > 0) {
//            this.collector.emit(TopologyControl.TOLL_NOTIFICATIONS_STREAM_ID, new Values(TOLL_NOTIFICATIONS_STREAM));
//        }
        this.collector.ack(tuple);
    }
