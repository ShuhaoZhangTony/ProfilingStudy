        ...
        for(ct2 in myseq){
          for(ct3 in myseq){
            ...
            if(file.exists(mypath)) {
              ...
              current_arg<-sprintf("application:%s, opt:%s,  batch:%s, %s_%s_%s",app,opt,bt,ct1,ct2,ct3)
      ...
      for(ct1 in myseq){
        for(ct2 in myseq){
          ...
          if(file.exists(mypath)) {
            setwd(mypath)
            current_time<-scan("sink.txt",nlines=1, quiet=TRUE)
            current_arg<-sprintf("application:%s,opt:%s, batch:%s, %s_%s",app,opt,bt,ct1,ct2)
            if(min_execution_time >current_time && current_time >0){
              min_execution_time<-current_time
              min_argument<-current_arg
            }
          ...
          for(ct3 in myseq){
            for(ct4 in myseq){
              ...
              if(file.exists(mypath)) {
                setwd(mypath)
                current_time<-scan("sink.txt",nlines=1, quiet=TRUE)
                current_arg<-sprintf("application:%s,opt:%s, batch:%s, %s_%s_%s_%s",app,opt,bt,ct1,ct2,ct3,ct4)
...
# 
# c("execution time"), # puts text in the legend 
#  
