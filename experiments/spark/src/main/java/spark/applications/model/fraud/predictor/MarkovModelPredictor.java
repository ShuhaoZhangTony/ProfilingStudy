...
import static spark.applications.constants.FraudDetectionConstants.DEFAULT_MODEL;
...
import spark.applications.util.data.Pair;
...
public class MarkovModelPredictor extends ModelBasedPredictor implements Serializable {
	...
	private static final long serialVersionUID = 2906230634828385854L;
    ...
    public MarkovModelPredictor(PredictionConfig conf) {
        String mmKey = conf.getModelKey();
        String model;
        ...
        localPredictor = conf.isLocalPredictor();

        if (localPredictor) {
            stateSeqWindowSize = conf.getStateSeqWindowSize();
        ...
        stateOrdinal = conf.getStateOrdinal();
        ...
        //metric threshold
        metricThreshold = conf.getMetricThreshold();
    }
    ...
    public Prediction execute(String entityID, String record) {
        ...
        Prediction prediction = new Prediction(entityID, score, stateSeq, (score > metricThreshold));//(..,..,..,isoutlier)
