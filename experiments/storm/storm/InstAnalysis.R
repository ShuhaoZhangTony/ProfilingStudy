...
for(fnt in start_fun:end_fun){
  ...
  if(fnt==start_fun){
    ...
    plot(ecdf1, verticals=TRUE, do.points=FALSE,xlim=c(0,50000), ylim=c(0,1))
    abline(v=32000,col="green")
    }
  else{plot(ecdf1, verticals=TRUE, do.points=FALSE,add=TRUE)
    }
...
##method 2
for(app in 4: 10){
    s<-paste(sprintf("C:/Users/szhang026/Documents/Profile-experiments/storm-tracing/log/genlib/%s_gaps",app),".txt", sep="")
    print(s)
    res <- scan(s)
    ecdf1 <- ecdf(res)
    if(app==4){
      plot(ecdf1, verticals=TRUE, do.points=FALSE,xlim=c(0,1000000))
...
for(app in c(4:8,10)){
  for(fnt in start_fun:end_fun){
    ...
    if(fnt==start_fun){
      if(app==4)
        plot(ecdf1, main="storm-platform",xlab="footprint (KB)", ylab = "CDF", verticals=TRUE, do.points=FALSE,xlim=c(0,50000), ylim=c(0,1), col='red')
      if(app==5)
        plot(ecdf1, verticals=TRUE, do.points=FALSE,xlim=c(0,200000), col='blue',add=TRUE)
      if(app==6)
        plot(ecdf1, verticals=TRUE, do.points=FALSE,xlim=c(0,200000), col='yellow',add=TRUE)
      if(app==7)
        plot(ecdf1, verticals=TRUE, do.points=FALSE,xlim=c(0,200000), col='green',add=TRUE)
      if(app==8)
        plot(ecdf1, verticals=TRUE, do.points=FALSE,xlim=c(0,200000), col='orange',add=TRUE)
      if(app==9)
        plot(ecdf1, verticals=TRUE, do.points=FALSE,xlim=c(0,200000), col='brown',add=TRUE)
      if(app==10)
        plot(ecdf1, verticals=TRUE, do.points=FALSE,xlim=c(0,200000), col='gray',add=TRUE)
      
      abline(v=32000,col="black")
    }
    
    else{
      if(app==4)
        plot(ecdf1, verticals=TRUE, do.points=FALSE,xlim=c(0,50000), col='red',add=TRUE)
      if(app==5)
        plot(ecdf1, verticals=TRUE, do.points=FALSE,xlim=c(0,200000), col='blue',add=TRUE)
      if(app==6)
        plot(ecdf1, verticals=TRUE, do.points=FALSE,xlim=c(0,200000), col='yellow',add=TRUE)
...
for(app in c(4:4)){
  for(fnt in start_fun:end_fun){
    ...
    if(fnt==start_fun){
      if(app==4)
        plot(ecdf1, main="storm-platform function",xlab="log-footprint (KB)", xlim=c(0,20), ylab = "PDF", col='red')
      if(app==5)        
...
for(app in c(4:4)){
  for(fnt in start_fun:end_fun){
    ...
    if(fnt==start_fun){
      if(app==4)
        plot(ecdf1, main="storm-app function",xlab="log-footprint (KB)", xlim=c(0,20), ylab = "PDF", col='red')
      if(app==5)        
        lines(ecdf1,  col='blue')
      if(app==6)
        lines(ecdf1,  col='yellow')
...
legend("topleft", legend = c("wc","fd","lg","sd","vs","tm","lr"), col=c('red','blue','yellow','green','orange','brown','gray'), pch=1) # optional legend


##method 4 all cdf seperate
start_fun=0
end_fun=2000
for(app in c(4:4)){
  for(fnt in start_fun:end_fun){
